...
void output(wstring startName, wstring endName,int type) {
	...
	dijkstra(start->getID(), distanceFromSource, predecessor, line,type);
	int minTime = INT_MAX;
...
void dijkstra(int sourceVertex, int distanceFromSource[], int predecessor[], busLine* line[],int type) {
	//Dijkstra算法
	...
	for (map<int, edge>::iterator i = aList[sourceVertex].begin(); i != aList[sourceVertex].end(); i++) {
		int ID = (*i).first;
		if (type == timeNoWait) {
			distanceFromSource[ID] = (*i).second.nWeight;
			line[ID] = (*i).second.nLine;
		}
		else if (type == timeWithWait) {
			distanceFromSource[ID] = (*i).second.wWeight;
			line[ID] = (*i).second.wLine;
		}
	...
	while (!newReachableVertices.empty()) {
		...
		for (map<int, edge>::iterator i = aList[v].begin(); i != aList[v].end(); i++) {
			int ID = (*i).first;
			if (type == timeNoWait) {
				if (distanceFromSource[ID] > distanceFromSource[v] + (*i).second.nWeight) {
					distanceFromSource[ID] = distanceFromSource[v] + (*i).second.nWeight;
					if (predecessor[ID] == -1)
						newReachableVertices.push_front(ID);
					predecessor[ID] = v;
					line[ID] = (*i).second.nLine;
				...
				if ((*i).second.times.find(line[v]) != (*i).second.times.end()) {
					...
					if (weight > (*i).second.wWeight) {
						weight = (*i).second.wWeight;
						lastLine = (*i).second.wLine;
					}
					else
						lastLine = line[v];
				}
